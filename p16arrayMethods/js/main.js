...
/*
    ...
    solicitud = prompt ('ingrese el producto buscado, ingrese un producto')

    const productsMapped = products.filter((product) => {
...
*/
    /*
    preguntar el preció y si es divisible entre 2 retornar solo descripción sino, 
    regresar el objeto normal

    preguntar si el preció es menor a 20 y si lo es agregar la propiedad 
    "ican: true" al objeto 
    
    regresar el objeto en formato de string con formato 'tipo - descripción - precio' 
    (el tipo debe venir en español y al precio agregarle $)

    */

  /*  const firstExercise = products.map((product) => {
...
/*
const secondExercise = products.map((product) => {
...
const thirdExercise = products.map((product) => {
        ...
        if (product.type == 'chip'){
            return `papas - ${product.description} - ${product.price}`
        }
    })
...
const xyz = products.reduce((accum, product) => {
        if (product.type === "beer"){ 
